...
#------------------------------------------------------------------------------
    ...
    def getU(self):
        ...
        if not hasattr(self, 'U'):
            ...
            self.U[0] = 0.125 * (3. * self.Q[0, 0] + 3. * self.Q[1, 1] +
                                 2. * self.Q[0, 1] + 4. * self.Q[2, 2])
            self.U[1] = 0.5 * (self.Q[0, 0] - self.Q[1, 1])
            self.U[2] = 0.125 * (self.Q[0, 0] + self.Q[1, 1] -
                                 2. * self.Q[0, 1] - 4. * self.Q[2, 2])
            self.U[3] = 0.125 * (self.Q[0, 0] + self.Q[1, 1] +
                                 6. * self.Q[0, 1] - 4. * self.Q[2, 2])
...
#------------------------------------------------------------------------------
    ...
    def getV(self):
        ...
        if not hasattr(self, 'V'):
            ...
            self.V[0] = 0.125 * (3. * self.S[0, 0] + 3. * self.S[1, 1] +
                                 2. * self.S[0, 1] + self.S[2, 2])
            self.V[1] = 0.5 * (self.S[0, 0] - self.S[1, 1])
            self.V[2] = 0.125 * (self.S[0, 0] + self.S[1, 1] -
                                 2. * self.S[0, 1] - self.S[2, 2])
            self.V[3] = 0.125 * (self.S[0, 0] + self.S[1, 1] +
                                 6. * self.S[0, 1] - self.S[2, 2])
...
#------------------------------------------------------------------------------
    ...
    def getFITsaiWu(self, sigma):
        ...
        a = (f11 * sigma[0]**2 + f22 * sigma[1]**2 + f66 * sigma[
            2]**2 + 2 * f12 * sigma[0] * sigma[1])
...
#------------------------------------------------------------------------------
    ...
    def getFIHashin80(self, sigma):
        ...
        if sigma[1] >= 0:
            ...
            FIm = ((sigma[1] / (2 * self.Sl))**2 +
                   ((self.Yc / (2 * self.Sl))**2 - 1.0) * sigma[1] / self.Yc +
                   (sigma[2] / self.Sl)**2)
...
#------------------------------------------------------------------------------
    ...
    def getFILarc03(self, sigma):
        ...
        for i in range(18):
            ...
            aa = Macauley(-sigma[1] * cos(alpr) *
                          (sin(alpr) - etaT * cos(alpr)))

            if (aa > tauTeff):
                tauTeff = aa

            aa = Macauley(
                cos(alpr) * abs(sigma[2] + etaL * sigma[1] * cos(alpr)))
        ...
        phi = (abs(sigma[2]) +
               (self.G12 - self.Xc) * phiC) / (self.G12 + sigma[0] - sigma[1])
        ...
        if sigma[1] >= 0:
            FIm = ((1.0 - self.g) * (sigma[1] / YTis) + self.g *
                   (sigma[1] / YTis)**2 + (sigma[2] / SLis)**2)
        else:
            if sigma[0] < self.Yc:
        ...
        if sigma[0] >= 0:
            ...
            if sigmam[1] < 0:
                ...
                FIf = ((1.0 - self.g) * (sigmam[1] / YTis) + self.g *
                       (sigmam[1] / YTis)**2 + (sigmam[2] / SLis)**2)
...
#------------------------------------------------------------------------------
    ...
    def __str__(self):
        ...
        for i, lay in enumerate(self.layers):
            msg += "   {:4}   {:4}   {:4}   {:4}\n".format(
                i, lay.thick, lay.theta, lay.name)
...
#------------------------------------------------------------------------------
    ...
    def getA(self):
        ...
        for i, layer in enumerate(self.layers):
            ...
            self.A += (self.materials[name].getQbar(theta) *
                       (self.h[i + 1] - self.h[i]))
...
#------------------------------------------------------------------------------
    ...
    def getB(self):
        ...
        for i, layer in enumerate(self.layers):
            ...
            self.B += (0.5 * self.materials[name].getQbar(theta) *
                       (self.h[i + 1]**2 - self.h[i]**2))
...
#------------------------------------------------------------------------------
    ...
    def getD(self):
        ...
        for i, layer in enumerate(self.layers):
            ...
            self.D += (1.0 / 3.0 * self.materials[name].getQbar(theta) *
                       (self.h[i + 1]**3 - self.h[i]**3))
...
#------------------------------------------------------------------------------
    ...
    def getTs(self):
        ...
        for i, layer in enumerate(self.layers):
            ...
            Qalpha = dot(self.materials[name].getQbar(theta),
                         self.materials[name].getAlpha(theta))
...
#------------------------------------------------------------------------------
    ...
    def getElastic(self):
        ...
        Ex = (self.A[0, 0] * self.A[1, 1] -
              self.A[0, 1] * self.A[0, 1]) / (self.thick * self.A[1, 1])
        Ey = (self.A[0, 0] * self.A[1, 1] -
              self.A[0, 1] * self.A[0, 1]) / (self.thick * self.A[0, 0])
