...
A flat, simply supported panel with dimensions $a = 400 \, \text{mm}$
and $b = 600 \, \text{mm}$ is loaded by a point load with magnitude
$F = 1200 \, \text{N}$ at the point $x_0 = 150 \, \text{mm}$ and
$y_0 = 350 \, \text{mm}$. The panel is made of 6 layers of UCHSC200_SE84
UD material (see Canvas for the properties), stacked in the sequence
$[0, 90, 0]_S$.

Calculate the out-of-plane displacement of the panel. Plot the results
as a contour plot.

**Solution**

Create the transverse isotropic material model and the laminate.
Calculate the $\mathbf{D}$ matrix and the parameters $D_1$, $D_2$, and
$D_3$.
...
Next, the dimensions of the panel and the loacation and magnitude of the
point load are entered.
...
With these variables, the terms $\mathbb{A}$ and $\mathbb{B}$ can be
calculated. In the analysis we will use the first 5 terms in both $x$
and $y$ direction, i.e. $m=1..5$ and $n=1..5$.
...
Note that we create numpy matrices of dimensions 6 by 6 for this. The
first row and column remain unused.

The displacement field $w(x,y)$ and the distributed load can be written as:
...
The load $q(x, y)$ can be plotted as a 3D surface plot or a contour
plot.
